...
`start-snaptask.command` をダブルクリックすると、空いているポート（3000〜3020）を選んで起動し、ブラウザも自動で開きます。手動で起動する場合は、プロジェクト内で `pnpm dev` を実行し、表示されたURLを開きます。すでに開いている古いタブではなく、起動時に表示されたURLを使ってください。
