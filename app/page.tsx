...
export default function Home() {
  ...
  return <main className={`snap-shell ${isPublicApp ? 'public-api-only' : ''}`}>
    ...
    {screen === 'account' && <section className="snap-page account-page"><button className="back-link" onClick={() => setScreen('home')}>← ホームに戻る</button><p className="kicker">ACCOUNT</p><h1>アカウント登録</h1><p className="intro">名前とメールアドレスを登録して、SnapTaskの利用者情報を保存できます。</p><div className="account-card"><div className="account-card-heading"><span className="account-avatar-large">{profile?.displayName.slice(0, 1) || 'S'}</span><div><b>{profile ? '登録済みのプロフィール' : '自分のプロフィールを作る'}</b><small>{profile ? 'この端末に保存されています' : 'まずは名前とメールだけでOK'}</small></div></div><form className="account-form" onSubmit={saveProfile}><label>表示名<input value={profileName} onChange={event => setProfileName(event.target.value)} placeholder="例：こう" autoComplete="name" maxLength={30} /></label><label>メールアドレス<input type="email" value={profileEmail} onChange={event => setProfileEmail(event.target.value)} placeholder="例：you@example.com" autoComplete="email" /></label><button className="save-button" type="submit">{profile ? 'プロフィールを更新する' : 'アカウント情報を保存する'} →</button>{profileMessage && <small className="account-message" role="status" aria-live="polite">{profileMessage}</small>}</form></div><div className="account-note"><b>現在の保存方式</b><span>このMVPではアカウント情報と学習データをこの端末のブラウザに保存します。複数端末でのログイン・同期を行う本番認証は、後からSupabaseやFirebaseを接続して追加できます。</span></div></section>}
