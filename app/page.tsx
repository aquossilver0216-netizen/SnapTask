...
export default function Home() {
  ...
  const [focusMinutes, setFocusMinutes] = useState(30); const [focusMinutesInput, setFocusMinutesInput] = useState('30');
  ...
  function updateFocusMinutes(value: string) { setFocusMinutesInput(value); const next = safeMinutes(value); if (!next) return; setFocusMinutes(next); try { localStorage.setItem('snaptask-focus-minutes', String(next)); } catch { /* 保存できなくてもその場の設定は維持 */ } }
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      try {
        ...
        const savedFocusMinutes = safeMinutes(localStorage.getItem('snaptask-focus-minutes')); if (savedFocusMinutes) { setFocusMinutes(savedFocusMinutes); setFocusMinutesInput(String(savedFocusMinutes)); }
