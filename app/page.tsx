...
type Screen = 'home' | 'add' | 'english' | 'share' | 'photos';
...
export default function Home() {
  ...
  const [tasks, setTasks] = useState<Task[]>(starterTasks); const [vocab, setVocab] = useState<Vocab[]>(starterVocab); const [activity, setActivity] = useState<Activity>({}); const [photos, setPhotos] = useState<SavedPhoto[]>([]); const [selectedPhoto, setSelectedPhoto] = useState<SavedPhoto | null>(null);
  ...
  function exportData() { const blob = new Blob([JSON.stringify({ tasks, vocab, deckNames, activity, weeklyGoal, wrongIds, photos }, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `snaptask-backup-${dateKey(new Date())}.json`; link.click(); URL.revokeObjectURL(url); setBackupMessage('バックアップを保存しました'); }
