...
import { AuthSession, fetchRemoteData, hydrateRemotePhotoUrls, isSupabaseConfigured, readGoogleSessionFromUrl, refreshSession, saveRemoteData, signIn, signOut, signUp, startGoogleSignIn, uploadRemotePhoto } from '../lib/supabase';
...
export default function Home() {
  ...
  return <main className={`snap-shell ${isPublicApp ? 'public-api-only' : ''}`}>
    ...
    {screen === 'account' && <section className="snap-page account-page"><button className="back-link" onClick={() => setScreen('home')}>← ホームに戻る</button><p className="kicker">ACCOUNT</p><h1>アカウント</h1><p className="intro">ログインすると、課題・暗記カード・学習記録を複数端末で同期できます。</p>{authSession ? <div className="account-card"><div className="account-card-heading"><span className="account-avatar-large">{profile?.displayName.slice(0, 1) || 'S'}</span><div><b>{profile?.displayName || 'SnapTaskユーザー'}</b><small>{authSession.user.email}</small></div></div><p className="sync-status">● {syncMessage || 'クラウド同期が有効です'}</p><button className="outline-button" onClick={handleSignOut} disabled={authBusy}>ログアウト</button></div> : supabaseReady ? <div className="account-card"><div className="account-card-heading"><span className="account-avatar-large">S</span><div><b>{authMode === 'signup' ? '無料アカウントを作成' : 'SnapTaskにログイン'}</b><small>Googleなら確認メールなしで同期</small></div></div><button className="google-button" type="button" onClick={handleGoogleSignIn} disabled={authBusy}>G&nbsp;&nbsp;Googleでログイン</button><p className="auth-divider"><span>またはメールでログイン</span></p><form className="account-form" onSubmit={submitAuth}>{authMode === 'signup' && <label>表示名<input value={profileName} onChange={event => setProfileName(event.target.value)} placeholder="例：こう" autoComplete="name" maxLength={30} /></label>}<label>メールアドレス<input type="email" value={profileEmail} onChange={event => setProfileEmail(event.target.value)} placeholder="例：you@example.com" autoComplete="email" /></label><label>パスワード<input type="password" value={authPassword} onChange={event => setAuthPassword(event.target.value)} placeholder="6文字以上" autoComplete={authMode === 'signup' ? 'new-password' : 'current-password'} /></label><button className="save-button" type="submit" disabled={authBusy}>{authBusy ? '処理中…' : authMode === 'signup' ? 'メールで登録する →' : 'メールでログインする →'}</button>{authMessage && <small className="account-message" role="status" aria-live="polite">{authMessage}</small>}</form><button className="auth-toggle" onClick={() => { setAuthMode(authMode === 'signup' ? 'signin' : 'signup'); setAuthMessage(''); }}>{authMode === 'signup' ? 'すでにアカウントがある？メールでログイン' : '初めて使う？メールで新規登録'}</button></div> : <div className="account-card"><div className="account-card-heading"><span className="account-avatar-large">S</span><div><b>Supabase接続待ち</b><small>本番ログインを有効にするには環境変数が必要です</small></div></div><p className="account-note">Vercelに <code>NEXT_PUBLIC_SUPABASE_URL</code> と <code>NEXT_PUBLIC_SUPABASE_ANON_KEY</code> を設定して再デプロイすると、ここからログインできます。</p><form className="account-form" onSubmit={saveProfile}><label>表示名<input value={profileName} onChange={event => setProfileName(event.target.value)} placeholder="例：こう" autoComplete="name" maxLength={30} /></label><label>メールアドレス<input type="email" value={profileEmail} onChange={event => setProfileEmail(event.target.value)} placeholder="例：you@example.com" autoComplete="email" /></label><button className="save-button" type="submit">この端末にプロフィールを保存 →</button>{profileMessage && <small className="account-message" role="status" aria-live="polite">{profileMessage}</small>}</form></div>}</section>}
