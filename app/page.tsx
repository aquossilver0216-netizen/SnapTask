...
import { AuthSession, fetchRemoteData, isSupabaseConfigured, refreshSession, saveRemoteData, signIn, signOut, signUp } from '../lib/supabase';

type Task = { id: string; title: string; subject: string; dueDate: string; body: string; done: boolean };
type SavedPhoto = { id: string; name: string; dataUrl: string; createdAt: string; kind: 'task' | 'memory' };
...
function demoTasksFor(today: string): Task[] { return [
  ...
  { id: 'task-2', title: '英語 長文プリント', subject: '英語', dueDate: shiftedDate(today, 5), body: '本文を読み、設問に答える', done: false },
]; }
...
function normalizeTasks(value: unknown): Task[] { if (!Array.isArray(value)) return []; return value.map((item, index) => { const row = recordOf(item); return { id: String(row.id ?? `task-${index + 1}`), title: String(row.title ?? '').trim(), subject: String(row.subject ?? '教科未設定').trim() || '教科未設定', dueDate: String(row.dueDate ?? '').trim(), body: String(row.body ?? '').trim(), done: row.done === true }; }).filter(task => task.title); }
...
function normalizePhotos(value: unknown): SavedPhoto[] { if (!Array.isArray(value)) return []; return value.map((item, index) => { const row = recordOf(item); const kind: SavedPhoto['kind'] = row.kind === 'memory' ? 'memory' : 'task'; return { id: String(row.id ?? `photo-${index + 1}`), name: String(row.name ?? '撮影した写真').trim() || '撮影した写真', dataUrl: String(row.dataUrl ?? ''), createdAt: String(row.createdAt ?? ''), kind }; }).filter(photo => photo.dataUrl); }
...
function FlashStudyPanel({ cards, index, known, revealed, finished, onReveal, onAnswer, onRestart, onClose }: { cards: Vocab[]; index: number; known: number; revealed: boolean; finished: boolean; onReveal: () => void; onAnswer: (known: boolean) => void; onRestart: () => void; onClose: () => void }) {
  ...
  return <div className="flash-panel" role="dialog" aria-modal="true" aria-label="一周学習"><button className="flash-close" onClick={onClose}>× 閉じる</button>
    {finished ? <div className="quiz-result"><p className="kicker">ROUND COMPLETE</p><strong>{known}/{cards.length}</strong><p>一周完了！覚えたカードは {known} 枚。もう一度にしたカードは「まちがい」に残っています。</p><button onClick={onRestart}>もう一度学習</button></div> : card ? <><div className="quiz-progress"><span>{index + 1} / {cards.length}枚</span><span>覚えた {known}</span></div><div className="flash-card"><p className="flash-term">{card.term}</p>{revealed ? <div className="flash-answer"><p>{card.meaning}</p><div className="flash-actions"><button onClick={() => onAnswer(false)}>もう一度</button><button onClick={() => onAnswer(true)}>覚えた</button></div></div> : <button className="reveal-button" onClick={onReveal}>説明を見る</button>}</div></> : <div className="empty-state"><b>カードがありません</b><span>先に暗記カードを追加しよう。</span></div>}
  </div>;
}
...
export default function Home() {
  ...
  const [profileName, setProfileName] = useState(''); const [profileEmail, setProfileEmail] = useState(''); const [profileMessage, setProfileMessage] = useState('');
  const [authSession, setAuthSession] = useState<AuthSession | null>(null); const [authMode, setAuthMode] = useState<'signup' | 'signin'>('signup'); const [authPassword, setAuthPassword] = useState(''); const [authBusy, setAuthBusy] = useState(false); const [authMessage, setAuthMessage] = useState(''); const [syncMessage, setSyncMessage] = useState(''); const [authHydrated, setAuthHydrated] = useState(false);
    ...
    const timer = window.setTimeout(() => {
      ...
      try {
        ...
        if (demoMode) {
          ...
          const savedProfile = normalizeProfile(JSON.parse(localStorage.getItem(PROFILE_STORAGE_KEY) ?? 'null')); if (savedProfile) { setProfile(savedProfile); setProfileName(savedProfile.displayName); setProfileEmail(savedProfile.email); }
  ...
  async function hydrateRemoteSession(session: AuthSession) {
    ...
    try {
      ...
      if (remote && (Array.isArray(data.tasks) || Array.isArray(data.vocab))) {
        ...
        setTasks(nextTasks); setVocab(nextVocab); setDeckNames(nextDecks.length ? nextDecks : defaultDeckNames); setActivity(nextActivity); setWrongIds(nextWrongIds); if (Number.isFinite(nextGoal) && nextGoal > 0) setWeeklyGoal(nextGoal);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void saveRemoteData(authSession, { tasks, vocab, deckNames, activity, weeklyGoal, wrongIds }).then(() => setSyncMessage('クラウドに同期しました。')).catch(() => setSyncMessage('同期に失敗しました。次回もう一度試します。'));
    }, 1200);
    return () => window.clearTimeout(timer);
  }, [mounted, authSession, authHydrated, tasks, vocab, deckNames, activity, weeklyGoal, wrongIds]);
  ...
  const quizOptions = useMemo(() => {
    ...
    const shift = quizQuestion.id.split('').reduce((sum, char) => sum + char.charCodeAt(0), 0) % options.length;
  ...
  async function submitAuth(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setAuthBusy(true); setAuthMessage(''); setSyncMessage('');
    const email = profileEmail.trim().toLocaleLowerCase(); const password = authPassword; const displayName = profileName.trim();
    if (!/^\S+@\S+\.\S+$/.test(email)) { setAuthMessage('メールアドレスの形式を確認してね。'); setAuthBusy(false); return; }
    if (password.length < 6) { setAuthMessage('パスワードは6文字以上で入力してね。'); setAuthBusy(false); return; }
  ...
  function saveProfile(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const displayName = profileName.trim(); const email = profileEmail.trim().toLocaleLowerCase(); if (displayName.length < 1 || displayName.length > 30) { setProfileMessage('名前は1〜30文字で入力してね。'); return; } if (!/^\S+@\S+\.\S+$/.test(email)) { setProfileMessage('メールアドレスの形式を確認してね。'); return; } const next: UserProfile = { id: profile?.id ?? newId('user'), displayName, email, createdAt: profile?.createdAt ?? new Date().toISOString() }; setProfile(next); setProfileName(next.displayName); setProfileEmail(next.email); localStorage.setItem(PROFILE_STORAGE_KEY, JSON.stringify(next)); setProfileMessage('アカウント情報を保存しました。'); }
  ...
  function addManualTask(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!manualTitle.trim()) { setManualMessage('課題名を入力してください。'); return; } const task = { id: newId('task'), title: manualTitle.trim(), subject: manualSubject.trim() || '教科未設定', dueDate: manualDue, body: manualBody.trim(), done: false }; const merged = mergeTasks(tasks, [task]); if (merged.length === tasks.length) { setManualMessage('同じ課題はすでに登録されています。'); return; } saveTasks(merged); setManualTitle(''); setManualSubject(''); setManualDue(''); setManualBody(''); setManualMessage(''); setMessage('課題を追加しました！'); setScreen('home'); }
  function startQuiz(reviewOnly = false) { const source = reviewOnly ? wrongDeckWords : deckWords; const unique = source.filter((item, index, items) => items.findIndex(candidate => candidate.meaning.trim().toLocaleLowerCase() === item.meaning.trim().toLocaleLowerCase()) === index).slice(0, 10); if (reviewOnly ? !unique.length : unique.length < 4) { setEnglishMessage(reviewOnly ? 'まちがいカードがありません。まず4択テストに挑戦してみよう。' : '4択テストには、説明が異なるカードが4件以上必要です。'); return; } setQuizReviewOnly(reviewOnly); setQuizRoundIds(unique.map(item => item.id)); setQuizIndex(0); setQuizScore(0); setQuizSelected(null); setQuizFinished(false); setMemoryMode('quiz'); }
  ...
  return <main className={`snap-shell ${isPublicApp ? 'public-api-only' : ''}`}>
    <aside className="workspace-sidebar" aria-label="ワークスペース">
      ...
      <button className="sidebar-account" onClick={() => { setScreen('account'); setProfileMessage(''); }}><span className="sidebar-account-avatar">{profile?.displayName.slice(0, 1) || '＋'}</span><span><b>{profile?.displayName || 'アカウントを登録'}</b><small>{profile?.email || '端末に保存'}</small></span><i>›</i></button>
    </aside>
