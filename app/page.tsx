...
export default function Home() {
  ...
  function exportData() { const blob = new Blob([JSON.stringify({ tasks, vocab, deckNames, activity, weeklyGoal, wrongIds }, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `snaptask-backup-${dateKey(new Date())}.json`; link.click(); URL.revokeObjectURL(url); setBackupMessage('バックアップを保存しました'); }
  ...
  function importData(event: ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0]; event.target.value = ''; if (!file) return; const reader = new FileReader(); reader.onload = () => { try { const data = JSON.parse(String(reader.result)) as { tasks?: unknown; vocab?: unknown; deckNames?: unknown; activity?: unknown; weeklyGoal?: unknown; wrongIds?: unknown }; if (!Array.isArray(data.tasks) || !Array.isArray(data.vocab)) throw new Error('バックアップ形式が違います'); const nextTasks = normalizeTasks(data.tasks); const nextVocab = normalizeVocab(data.vocab); const nextActivity = normalizeActivity(data.activity); const nextWrongIds = normalizeWrongIds(data.wrongIds); const storedDecks = Array.isArray(data.deckNames) ? data.deckNames.map(String).map(name => name.trim()).filter(Boolean) : []; const nextDecks = Array.from(new Set([...storedDecks, ...nextVocab.map(card => card.subject)])); setTasks(nextTasks); setVocab(nextVocab); setDeckNames(nextDecks.length ? nextDecks : defaultDeckNames); setWrongIds(nextWrongIds); setActivity(nextActivity); if (typeof data.weeklyGoal === 'number' && data.weeklyGoal > 0) setWeeklyGoal(data.weeklyGoal); localStorage.setItem('snaptask-tasks', JSON.stringify(nextTasks)); localStorage.setItem('snaptask-vocab', JSON.stringify(nextVocab)); localStorage.setItem('snaptask-decks', JSON.stringify(nextDecks.length ? nextDecks : defaultDeckNames)); localStorage.setItem('snaptask-wrong-cards', JSON.stringify(nextWrongIds)); localStorage.setItem('snaptask-activity', JSON.stringify(nextActivity)); setBackupMessage('バックアップを復元しました'); } catch { setBackupMessage('バックアップを読み込めませんでした'); } }; reader.readAsText(file); }
  ...
  async function copyShareLink() { const name = shareDeck || shareableDecks[0]; if (!name) { setShareMessage('共有できる暗記ページがありません。'); return; } try { await navigator.clipboard.writeText(deckShareUrl(name)); setShareMessage(`${name}の共有リンクをコピーしました。リンクを開くだけで取り込めます。`); } catch { setShareMessage('リンクをコピーできませんでした。もう一度試してね。'); } }
  async function openNativeShare() { const name = shareDeck || shareableDecks[0]; if (!name) { setShareMessage('共有できる暗記ページがありません。'); return; } const url = deckShareUrl(name); if (typeof navigator.share === 'function') { try { await navigator.share({ title: `${name}｜SnapTask暗記ページ`, text: `${name}の暗記カード（${vocab.filter(card => card.subject === name).length}枚）を共有します。`, url }); setShareMessage(`${name}を共有しました。`); return; } catch (error) { if (error instanceof DOMException && error.name === 'AbortError') { setShareMessage('共有をキャンセルしました。'); return; } } } await copyShareLink(); }
  async function copyDeck() { const name = shareDeck || shareableDecks[0]; if (!name) { setShareMessage('共有できる暗記ページがありません。'); return; } try { await navigator.clipboard.writeText(deckReadableText(name)); setShareMessage(`${name}の学習内容を読みやすい形式でコピーしました。`); } catch { setShareMessage('コピーできませんでした。共有リンクを使ってね。'); } }
  function downloadDeck() { const name = shareDeck || shareableDecks[0]; if (!name) { setShareMessage('共有できる暗記ページがありません。'); return; } const blob = new Blob([deckPayload(name)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `snaptask-${name}.json`; link.click(); URL.revokeObjectURL(url); setShareMessage(`${name}を共有ファイルにしました。`); }
