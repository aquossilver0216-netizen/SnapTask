...
export default function Home() {
  ...
  return <main className="snap-shell">
    ...
    <header className="snap-header"><button className="snap-brand" onClick={() => setScreen('home')}><span className="brand-dot">S</span><span><b>SnapTask</b><small>高校生のための提出物管理</small></span></button><div className="header-page-context">{screen === 'home' ? 'ホーム' : screen === 'add' ? '課題を追加' : screen === 'english' ? `暗記 / ${deck}` : screen === 'photos' ? '写真' : '共有'}</div><button className="header-search" onClick={() => setWorkspaceSearchOpen(true)} aria-label="検索">⌕</button><button className="header-add" onClick={() => setScreen('add')}>＋ 写真を追加</button></header>
