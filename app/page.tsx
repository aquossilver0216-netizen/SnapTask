...
export default function Home() {
  ...
  function renameDeck(currentName: string) { const value = window.prompt('単語帳の名前を変更', currentName); const nextName = value?.trim(); if (!nextName || nextName === currentName) return; if (decks.some(item => item !== currentName && item.toLocaleLowerCase() === nextName.toLocaleLowerCase())) { setEnglishMessage('同じ名前の単語帳がすでにあります。'); return; } saveVocab(vocab.map(card => card.subject === currentName ? { ...card, subject: nextName } : card)); setDeck(nextName); if (shareDeck === currentName) setShareDeck(nextName); setEnglishMessage(`単語帳を「${nextName}」に変更しました。`); }
