...
export default function Home() {
    ...
    const timer = window.setTimeout(() => {
      ...
      try {
        ...
        if (demoMode) {
          ...
          setTasks(nextTasks); setVocab([...starterVocab]); setActivity({}); setWrongIds([]); setWeeklyGoal(5); setTutorialStep(0); setShowGuide(true);
  ...
  function resetDemoData() { if (!window.confirm('課題・暗記カード・学習記録をデモ状態に戻します。保存済みデータは上書きされます。')) return; const nextTasks = demoTasksFor(todayKey || dateKey(new Date())); setTasks(nextTasks); setVocab([...starterVocab]); setActivity({}); setWrongIds([]); setWeeklyGoal(5); setSubject('すべて'); setDeck('英語'); setMemoryMode('list'); setReviewWrongOnly(false); setTutorialStep(0); setShowGuide(true); localStorage.setItem('snaptask-tasks', JSON.stringify(nextTasks)); localStorage.setItem('snaptask-vocab', JSON.stringify(starterVocab)); localStorage.setItem('snaptask-activity', '{}'); localStorage.setItem('snaptask-wrong-cards', '[]'); localStorage.setItem('snaptask-weekly-goal', '5'); localStorage.removeItem('snaptask-guide-seen'); setBackupMessage('デモデータに戻しました'); }
