...
export default function Home() {
  ...
  const [memoryMode, setMemoryMode] = useState<MemoryMode>('list'); const [quizIndex, setQuizIndex] = useState(0); const [quizScore, setQuizScore] = useState(0); const [quizSelected, setQuizSelected] = useState<string | null>(null); const [quizFinished, setQuizFinished] = useState(false);
  ...
  const quizOptions = useMemo(() => quizQuestion ? [quizQuestion, ...deckWords.filter(item => item.id !== quizQuestion.id).slice(0, 3)] : [], [deckWords, quizQuestion]);
  ...
  function answerQuiz(value: string) { if (quizSelected || !quizQuestion) return; setQuizSelected(value); if (value === quizQuestion.meaning) setQuizScore(score => score + 1); window.setTimeout(() => { if (quizIndex + 1 >= quizQuestions.length) setQuizFinished(true); else { setQuizIndex(index => index + 1); setQuizSelected(null); } }, 650); }
