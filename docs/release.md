...
- 「共有リンク」は暗記カードをURLハッシュへ埋め込む端末保存MVP。ログイン不要で友だちへ渡せる。端末の共有シートはこのリンクを送信し、「学習内容をコピー」はコードではなく読みやすいテキスト＋リンクをコピーする。公開リンクの取り消し・共有回数課金などを本番運用する場合は、認証・共有データベース・決済サーバーを追加する。
